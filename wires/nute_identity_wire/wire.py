...
class NUTEIdentityWire:
    WIRE_ID = "SW-NUTE-IDENTITY-001"
    VERSION = "0.1.0"
    ...
    def validate(self, box_result):
        ...
        if invalid_types:
            return {
                "valid": False,
                "error": interface["validation"]["failure_code"],
                "message": "Invalid identity field types.",
        ...
        return {
            "valid": True,
            "error": None,
            "message": "NUTE identity output is valid."
        }
...
if __name__ == "__main__":
    print(
        "NUTEIdentityWire is ready. "
        "It expects output from capability 'lis.identity.nute'."
    )